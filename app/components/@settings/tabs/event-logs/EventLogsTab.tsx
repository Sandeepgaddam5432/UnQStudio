            ...
            {log.details && (
              <>
                <button
                  ...
                  className="text-xs text-gray-500 dark:text-gray-400 hover:text-[var(--unq-elements-item-contentAccent)] dark:hover:text-[var(--unq-elements-item-contentAccent)] transition-colors"
                >
...
export function EventLogsTab() {
  ...
  const exportAsJSON = () => {
    try {
      ...
      a.href = url;
    a.download = `UnQStudio-event-logs-${new Date().toISOString()}.json`;
  ...
  const exportAsCSV = () => {
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
    a.download = `UnQStudio-event-logs-${new Date().toISOString()}.csv`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
  ...
  const exportAsPDF = () => {
    try {
      ...
      // Add subtitle with UnQStudio
      doc.setFontSize(12);
      doc.setFont('helvetica', 'normal');
      doc.text('UnQStudio - AI Development Platform', margin, 45);
      yPos = 70;
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        // Add footer text
        doc.text('Generated by UnQStudio', margin, doc.internal.pageSize.getHeight() - 10);
      ...
      // Save the PDF
      doc.save(`UnQStudio-event-logs-${new Date().toISOString()}.pdf`);
  ...
  const exportAsText = () => {
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
    a.download = `UnQStudio-event-logs-${new Date().toISOString()}.txt`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
  ...
  const ExportButton = () => {
    ...
    return (
      <DialogRoot open={isOpen} onOpenChange={handleOpenChange}>
        <button
          ...
          className={classNames(
            ...
            'bg-[#FAFAFA] dark:bg-[#0A0A0A]',
            'border border-[#E5E5E5] dark:border-[#1A1A1A]',
            'hover:bg-[var(--unq-elements-button-primary-background)]/10 dark:hover:bg-[var(--unq-elements-button-primary-background)]/20',
            'transition-all duration-200',
          )}
        >
          <span className="i-ph:download text-lg text-gray-500 dark:text-gray-400 group-hover:text-[var(--unq-elements-item-contentAccent)] transition-colors" />
          Export
        </button>
      ...
      <div className="text-xs text-unq-elements-textSecondary mt-0.5">
    ...
    <div className="flex h-full flex-col gap-6">
      <div className="flex items-center justify-between">
          ...
          <DropdownMenu.Portal>
            ...
            >
              {logLevelOptions.map((option) => (
                <DropdownMenu.Item
                  key={option.value}
                  className="group flex items-center px-4 py-2.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-[var(--unq-elements-button-primary-background)]/10 dark:hover:bg-[var(--unq-elements-button-primary-background)]/20 cursor-pointer transition-colors"
                  onClick={() => handleLevelFilterChange(option.value)}
                >
                  <div className="mr-3 flex h-5 w-5 items-center justify-center">
                    <div
                      className={classNames(option.icon, 'text-lg group-hover:text-[var(--unq-elements-item-contentAccent)] transition-colors')}
                      style={{ color: option.color }}
                    />
                  </div>
                  <span className="group-hover:text-[var(--unq-elements-item-contentAccent)] transition-colors">{option.label}</span>
                </DropdownMenu.Item>
              ))}
            </DropdownMenu.Content>
          </DropdownMenu.Portal>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <span className="i-ph:arrows-clockwise text-lg text-gray-500 dark:text-gray-400 group-hover:text-[var(--unq-elements-item-contentAccent)] transition-colors" />
            Refresh
          </button>
